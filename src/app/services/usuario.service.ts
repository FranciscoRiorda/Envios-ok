...
export class UsuarioService {
  ...
  constructor(private http: HttpClient,
    ...
    validarToken(): Observable<boolean> {
      ...
      return this.http.get(`${base_url}/login/renovar`, {
        ...
        map((resp: any) => {
          ...
          const {nombre, apellido, dni, telefono, email, domicilio, nacimiento, role, img = '', uid, terminos} = resp.usuario;
          this.usuario = new Usuario(nombre, apellido, dni, telefono, email, domicilio, nacimiento, '', img, role, uid, terminos);
    ...
    get terminos(): String {
      return this.usuario.terminos!
    }

    actualizarPerfil(data: {nombre: string, apellido: string, dni: string, telefono: string, email: string, domicilio: string, nacimiento: string, role: any, terminos: any}){
        ...
        .pipe(
          map( resp => {
            const usuarios = resp.usuarios.map(
              user => new Usuario(user.nombre, user.apellido, user.dni, user.telefono, user.email, user.domicilio, user.nacimiento, user.password, user.img, user.role, user.uid, user.terminos )).sort(function(a, b) {if (a.apellido > b.apellido) {return 1;} if (a.apellido < b.apellido) {return -1;} return 0;});
